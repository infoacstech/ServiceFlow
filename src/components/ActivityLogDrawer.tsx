        ...
        <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
          ...
          >
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 space-y-3 bg-white dark:bg-slate-900">
              ...
              {/* Entity category pills & Limit Selector */}
              <div className="flex items-center justify-between gap-2 overflow-x-auto pb-1 no-scrollbar text-xs">
                ...
                <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-lg shrink-0 border border-slate-200 dark:border-slate-700">
            ...
            {/* Showing status bar */}
            <div className="px-5 py-2 bg-slate-100/70 dark:bg-slate-800/40 border-b border-slate-200/60 dark:border-slate-800/60 flex items-center justify-between text-[11px] text-slate-500">
              <span>
                Showing <strong>{displayedLogs.length}</strong> of {allFilteredLogs.length} activities
              </span>
              {logLimit !== 'all' && allFilteredLogs.length > logLimit && (
                ...
                >
                  View All ({allFilteredLogs.length})
                </button>
              )}
            </div>
                  ...
                  {displayedLogs.map((log) => {
                    ...
                    return (
                      ...
                      >
                        {/* Timeline Dot Icon */}
                        <div
                          className={`absolute -left-6 top-0 w-5 h-5 rounded-full border-2 flex items-center justify-center shadow-xs transition-colors ${
                            isSelected
                        ...
                        {/* Card Content */}
                        <div
                          className={`rounded-2xl p-3.5 border shadow-xs transition-all space-y-2 ${
                            isSelected
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between gap-2">
                            ...
                            {isSelected && (
                              <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-200 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200 font-bold">
                                Selected
                              </span>
                            )}
                          </div>
                          ...
                          {isSelected && (
                            <div className="p-2 rounded-xl bg-white/80 dark:bg-slate-900/80 border border-indigo-200 dark:border-indigo-800/60 text-[10px] text-slate-600 dark:text-slate-300 space-y-1">
                              <div className="font-mono text-slate-400">ID: {log.id}</div>
                              {log.entityId && <div className="font-mono text-slate-400">Entity: {log.entityId}</div>}
                              <div className="text-indigo-600 dark:text-indigo-400 font-medium">
                                Exact ISO Time: {log.timestamp}
                              </div>
                            </div>
                          )}
