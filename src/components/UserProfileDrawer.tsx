...
export const UserProfileDrawer: React.FC<UserProfileDrawerProps> = ({
  ...
  const handleUpdateEmailSubmit = async (e: React.FormEvent) => {
    ...
    const currentCleanEmail = (currentUser.email || '').trim().toLowerCase();

    if (!cleanNewEmail || !cleanNewEmail.includes('@') || !cleanNewEmail.includes('.')) {
      setEmailUpdateError('Please provide a valid email address.');
      return;
    }

    if (cleanNewEmail === currentCleanEmail) {
      setEmailUpdateError('The new email address is identical to your current email address.');
      return;
    }

    if (cleanNewEmail === 'admin@serviflow.io' || cleanNewEmail === 'superadmin@serviflow.io') {
      setEmailUpdateError('This email is reserved for Platform Super Administrator.');
      return;
    }

    // Check duplicate in users list
    const isDuplicate = (users || []).some(
      (u) => u.id !== currentUser.id && (u.email || '').trim().toLowerCase() === cleanNewEmail
    );
    if (isDuplicate) {
      setEmailUpdateError('This email is already associated with another account on this system.');
      return;
    }

    // Verify current password
    const expectedPassword = currentUser.password || 'ServiFlow@123';
    if (currentUser.password && emailVerifyPassword !== expectedPassword) {
      setEmailUpdateError('Incorrect current password. Identity verification failed.');
      return;
    }
    ...
    try {
      await updateUserProfile(currentUser.id, { email: cleanNewEmail });
      showToast(`Sign-in email updated to ${cleanNewEmail}! Active session preserved.`, 'success');
    ...
    <div className="fixed inset-0 z-[99999] overflow-hidden animate-in fade-in duration-200" id="user-profile-drawer-root">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-0 sm:pl-6 md:pl-10 z-[100000] pointer-events-auto">
        ...
        >
          ...
          {/* Header */}
          <div className="py-2.5 px-3.5 sm:px-4 border-b border-slate-200/80 dark:border-slate-800 flex items-center justify-between bg-slate-50/90 dark:bg-slate-900/90 backdrop-blur-md shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-indigo-600 text-white flex items-center justify-center font-bold text-xs sm:text-sm shadow-sm overflow-hidden ring-2 ring-indigo-500/20 shrink-0">
                {currentUser?.avatar ? (
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-xs sm:text-sm font-black text-slate-900 dark:text-slate-100 truncate leading-tight">
                  {currentUser?.name || 'User Profile & Settings'}
                </h2>
                <p className="text-[11px] text-indigo-600 dark:text-indigo-400 font-semibold truncate flex items-center gap-1 mt-0.5">
                  <Building2 className="w-3 h-3 shrink-0" />
                  <span className="truncate">{currentBusiness?.name || 'ServiFlow Workspace'}</span>
                </p>
              </div>
            </div>
            <button
              ...
              onClick={onClose}
              className="p-1.5 sm:p-2 rounded-xl hover:bg-slate-200/80 dark:hover:bg-slate-800 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100 transition-colors cursor-pointer shrink-0 min-w-[36px] min-h-[36px] flex items-center justify-center"
              title="Close Panel"
          ...
          >
            ...
            {activeSection === 'profile' && (
              <div className="space-y-3.5 sm:space-y-4 animate-in fade-in duration-150">
                {/* Profile Photo & Quick Overview Card - Compact, Clean & Balanced */}
                <div className="p-3 sm:p-3.5 rounded-2xl bg-gradient-to-r from-indigo-50/70 via-slate-50 to-indigo-50/30 dark:from-indigo-950/30 dark:via-slate-900 dark:to-indigo-950/20 border border-indigo-100/90 dark:border-indigo-900/40 shadow-xs">
                  <div className="flex items-center gap-3 sm:gap-3.5">
                    {/* Interactive Avatar Container */}
                    <div className="relative group shrink-0">
                      <div className="w-13 h-13 sm:w-15 sm:h-15 rounded-2xl bg-indigo-600 text-white flex items-center justify-center font-black text-lg shadow-md overflow-hidden ring-2 ring-white dark:ring-slate-800 shrink-0">
                      ...
                      <button
                        ...
                        className="absolute -bottom-1 -right-1 p-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-md border border-white dark:border-slate-800 cursor-pointer transition-transform group-hover:scale-110 flex items-center justify-center"
                        title="Upload Photo"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                        ...
                        <span className="text-[9px] sm:text-[10px] px-2 py-0.5 rounded-full font-bold uppercase bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 shrink-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-1.5">
                        ...
                        <button
                          type="button"
                          onClick={() => fileInputRef.current?.click()}
                          className="px-2 py-1 text-[10px] font-bold text-indigo-700 dark:text-indigo-300 bg-white dark:bg-slate-800 border border-indigo-200 dark:border-indigo-800 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-950/60 transition-colors flex items-center gap-1 cursor-pointer"
                        >
                        ...
                        {(avatar || currentUser?.avatar) && (
                          <button
                            ...
                            className="px-2 py-1 text-[10px] font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg transition-colors flex items-center gap-1 cursor-pointer"
                          >
                ...
                <form onSubmit={handleSaveProfile} className="p-3.5 sm:p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 space-y-3 shadow-sm">
                  ...
                  {/* Protected Sign-in Email with Secure Change Option */}
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
                        Email Address (Sign-In Identifier)
                      </label>
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-semibold flex items-center gap-1 border border-slate-200 dark:border-slate-700">
                        <Shield className="w-3 h-3 text-indigo-600 dark:text-indigo-400" /> Protected Credential
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        disabled
                        className="flex-1 text-xs p-2.5 sm:p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-800/40 text-slate-600 dark:text-slate-400 font-medium cursor-not-allowed min-h-[42px]"
                      />
                      <button
                        type="button"
                        onClick={handleOpenEmailModal}
                        className="px-3 py-2 text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 border border-indigo-200 dark:border-indigo-800 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer min-h-[42px] shrink-0"
                        title="Change Sign-In Email Address"
                      >
                        <KeyRound className="w-3.5 h-3.5" /> Change
                      </button>
                    </div>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">
                      Primary sign-in credential. Requires password verification to modify.
                    </p>
                  </div>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-200/80 dark:border-slate-800">
              {/* Distinct System Configurations vs Business Profile */}
              <button
      ...
      {isEmailModalOpen && (
        <div className="fixed inset-0 z-[100005] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl p-4 sm:p-5 space-y-4">
            ...
            <form onSubmit={handleUpdateEmailSubmit} className="space-y-3.5">
              <div className="p-3 rounded-xl bg-indigo-50/70 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/50 text-xs text-indigo-900 dark:text-indigo-200 space-y-1">
                ...
                <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                  Changing your sign-in email updates the email address used to log into your account. Your active session and company records will be preserved seamlessly.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="px-3 py-2 rounded-xl bg-slate-100 dark:bg-slate-800/60 text-xs text-slate-600 dark:text-slate-400 font-mono">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (emailUpdateError) setEmailUpdateError('');
                  }}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      setEmailVerifyPassword(e.target.value);
                      if (emailUpdateError) setEmailUpdateError('');
                    }}
              ...
              {emailUpdateError && (
                <div className="p-2.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 text-rose-700 dark:text-rose-300 text-xs font-medium">
                  {emailUpdateError}
                </div>
              )}
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                <button
                  ...
                  className="px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer min-h-[40px]"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={isUpdatingEmail}
                  className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold shadow-sm transition-all cursor-pointer disabled:opacity-60 flex items-center gap-1.5 min-h-[40px]"
                >
                  <ShieldCheck className="w-4 h-4" />
                  {isUpdatingEmail ? 'Verifying & Updating...' : 'Confirm & Update Email'}
                </button>
              </div>
            </form>
          </div>
